...
export class TypesPlanesService {
  ...
  setForm(row): void{
    ...
    this.form.setValue({
      name: row.name,
      types_classes: row.types_classes,
  ...
  // Api
  get(request?){
    return this.http.get(`${this.baseUrl}/type-plane`, request /*, {"headers":  this.headers }*/ );
  }
  insert(request){
    return this.http.post(`${this.baseUrl}/type-plane`, request, { headers: this.headers } );
  }
  update(request,id){
    return this.http.put(`${this.baseUrl}/type-plane/${id}`, request, { headers: this.headers } );
  }
}
